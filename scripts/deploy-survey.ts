/**
 * Deploy the anonymous survey contract to a Midnight network (undeployed by
...
// Same hash shape the contract computes on-chain for `persistentHash<Vector<1, Bytes<32>>>([secretKey()])`.
...
function commitmentOf(secretKey: Uint8Array): Uint8Array {
  return persistentHash(commitmentDescriptor, [secretKey]);
}
...
const SurveyContract = await import(pathToFileURL(contractPath).href);

// ─── Roster generation ──────────────────────────────────────────────────────────

function generateRoster(): { secretKeys: Uint8Array[]; commitments: Uint8Array[] } {
  const secretKeys = Array.from({ length: ROSTER_SIZE }, () => crypto.randomBytes(32));
  const commitments = secretKeys.map(commitmentOf);
  return { secretKeys, commitments };
}
...
const compiledContract = CompiledContract.make('survey', SurveyContract.Contract).pipe(
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy anonymous survey to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      deployed = await deployContract(providers, {
        ...
        initialPrivateState: { secretKey: secretKeys[0] },
      });
      break;
    } catch (err: any) {
