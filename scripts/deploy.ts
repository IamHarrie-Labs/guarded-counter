/**
 * Deploy the guarded counter contract to a Midnight network (undeployed by
 * default; use --network preview|preprod for public networks).
 *
 * Non-interactive: run straight through once the wallet is funded.
 */
...
// The counter's guard key. Derived from the deployer's wallet seed so it's
// reproducible across runs without a second secret to keep track of — the
// same wallet always regenerates the same guard key. This is fine for a
// Level 1 demo; a real product would let the user pick their own key.
function deriveGuardKey(seed: string): Uint8Array {
  return new Uint8Array(crypto.createHash('sha256').update(`${seed}:guard-key`).digest());
}
...
const guardKey = deriveGuardKey(SEED);
const witnesses = {
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy guarded counter to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  if (unregisteredUtxos.length > 0) {
    ...
    //
    // The RPC websocket has been observed dropping mid-submission on Preview
    // ("disconnected ... Normal Closure") with no chain-level rejection — the
    // tx just never lands. Since the UTXOs stay unregistered when that
    // happens, redoing the whole recipe/sign/submit is safe to retry.
    const REGISTER_MAX_RETRIES = 5;
    const REGISTER_RETRY_DELAY_MS = 5000;
    for (let attempt = 1; attempt <= REGISTER_MAX_RETRIES; attempt++) {
      try {
        ...
        const isDropped = msg.includes('disconnected') || msg.includes('SubmissionError') || msg.includes('Normal Closure');
        if (!isDropped || attempt === REGISTER_MAX_RETRIES) throw err;
        console.log(`  ⏳ Submission dropped (attempt ${attempt}/${REGISTER_MAX_RETRIES}), retrying in ${REGISTER_RETRY_DELAY_MS / 1000}s...`);
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // empty here since counter.compact's constructor takes none.
      ...
      // The RPC node has been dropping the connection mid-submission on
      // Preview — sometimes the transaction still lands (we confirmed this
      // with the DUST-registration step: the balance in that log line was
      // updated even after this exact error), sometimes it doesn't. Either
      // way it isn't fatal — retry rather than aborting the whole deploy on
      // the very first flaky connection.
      const isSubmissionDropped =
        fullError.includes('SubmissionError') ||
        fullError.includes('Transaction submission') ||
        fullError.includes('disconnected from wss:') ||
        fullError.includes('No response received from RPC endpoint');
